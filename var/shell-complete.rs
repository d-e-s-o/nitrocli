...
fn generate_for_shell<W>(command: &str, shell: clap_generate::Shell, output: &mut W)
where
  W: io::Write,
{
  let mut app = nitrocli::Args::into_app();
  clap_generate::generate(shell, &mut app, command, output);
}
...
mod tests {
    ...
    W: ExactSizeIterator<Item = &'w str>,
  {
    ...
    generate_for_shell("nitrocli", clap_generate::Shell::Bash, &mut buffer);
